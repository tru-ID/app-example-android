...
class PhoneCheckAdapter {
    ...
    fun getPhoneCheckPostBody(phoneNumber: String): RequestBody {
        val phoneCheckPostJsonAdapter: JsonAdapter<PhoneCheckPost> = moshi.adapter(
    ...
    fun parsePhoneCheckResponse(response: String): PhoneCheck {
        val phoneCheckJsonAdapter: JsonAdapter<PhoneCheck> = moshi.adapter(
            PhoneCheck::class.java)
        return phoneCheckJsonAdapter.fromJson(response)!!
    }

    fun parsePhoneCheckResults(response: String): PhoneCheckResult {
        val phoneCheckResultJsonAdapter: JsonAdapter<PhoneCheckResult> = moshi.adapter(
            PhoneCheckResult::class.java)
